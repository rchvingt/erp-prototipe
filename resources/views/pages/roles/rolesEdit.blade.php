            ...
            <div class="col-xl">
                ...
                <div class="mb-6 card">
                    <div class="card-header d-flex flex-column justify-content-center">
                        <h5 class="mb-0">Form Edit Role Baru</h5>
                        <p>Set role permissions {{ $role->name }}</p>
                    </div>
                    <div class="card-body">

                        <!-- Add role form -->
                        {{-- {{ dd($role->id) }} --}}
                        <form class="row g-6" method="POST" action="{{ route('roles.update', $role->id) }}">

                            @csrf
                            @method('PUT')

                            <div class="col-12">
                                <label class="form-label" for="name="name"">Nama Role</label>
                                                ...
                                                <tr class="permission-group">
                                                    <td class="text-nowrap fw-medium text-heading">
                                                        <div class="form-check">
                                                            ...
                                                            <label class="form-check-label">{{ ucfirst($group) }}</label>
                                                        </div>
                                                    </td>
                                                    <td>
                                                            ...
                                                            @foreach ($groupPermissions as $permission)
                                                                <div class="mb-0 form-check me-4 me-lg-12">
                                                                    <input type="checkbox" name="permissions[]"
                                                                        class="form-check-input permission-checkbox permission-{{ str_replace(' ', '-', $group) }}"
                                                                        value="{{ $permission->id }}"
                                                                        data-group="{{ str_replace(' ', '-', $group) }}"
